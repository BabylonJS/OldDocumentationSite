...
If you want to change user inputs based on user preferences, customize one of the existing presets, or use your own input mecanisms, those camera have an input manager that is designed for those advanced scenarios. Read [customizing camera inputs](http://doc.babylonjs.com/tutorials/19._Customizing_Camera_Inputs) to read more about tweaking inputs on your cameras.
