...
But you can also use a RenderTargetTexture as source of refraction when you deal with a flat object:
```javascript
var renderTargetTexture = new BABYLON.RenderTargetTexture("th", 1024, scene);
renderTargetTexture.renderList.push(yellowSphere);
renderTargetTexture.renderList.push(greenSphere);
renderTargetTexture.renderList.push(ground);
mainMaterial.diffuseColor = new BABYLON.Color3(1, 1, 1);
mainMaterial.refractionTexture = renderTargetTexture;
```
See a demo here: http://www.babylonjs-playground.com/#22KZUW#12
