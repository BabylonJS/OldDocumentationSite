...
---
Some of Babylon.js's cameras respond to user inputs. This is especially true for ArcRotateCamera and FreeCamera. Babylon.js v2.4 introduced a different way to manage camera inputs to provide an approach oriented toward composability of inputs. Those cameras now uses an input manager and each input could be seen as a plugin that is specific to this camera family, and to a given input type (mouse, keyboard, gamepad, device orientation, ...).

Using input manager you could add, remove, enable, or disable any input available for the camera. You could also implement your own input mecanism or override the existing one very easily.

The input manager is available on those cameras through a property called "inputs".
...
Input manager of both ArcRotateCamera and FreeCamera expose short-hand functions for adding built-ins input.  
...
If you wish, you could also add an input by providing an instance of your input (we will cover how to implement your own input at the end of this article).
...
If you want to disable an input temporarily, you could call "detachControl" directly on the input like this
...
You could then call "attachInput" when you want to turn it on again.
```javascript
camera.inputs.attachInput(camera.inputs.attached.mouse);
```

## Removing inputs
Sometimes you want very specific input mecanism. The best approach in such case is probably to clear all inputs and add only those you may want in your scene.
...
You could also remove a single input from your input manager. You could remove them by instance, or by Type name
```javascript
var camera = new BABYLON.FreeCamera("sceneCamera", new BABYLON.Vector3(0, 1, -15), scene);
//remove by instance
