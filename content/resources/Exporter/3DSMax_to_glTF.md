...
Lights are not supported in glTF 2.0. An empty node is exported in place of light only when it is relevant to do so (when a light has a mesh or a camera as descendant).  Lights can be enabled by turning on the KHR_lights_punctual extension.
